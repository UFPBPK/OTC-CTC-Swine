...
Pars_CTC <- c(
  ...
  PB          = 0.55,
  Ka          = 0.08,#Oral administration with water
  KurineC     = 0.001,           
  Kint        = 0.65,
  KbileC      = 0.45,
  Kst         = 0.008,
...
#Intestinal absorption rate constant (/h), oral administration with water
Ka.mean           = 0.08
...
#Volume of tissues (fraction of Body weight)
VLCa.CV         = 0.162                                  
VKCa.CV         = 0.297 
VMCa.CV         = 0.073 
VFCa.CV         = 0.172                                     
...
#Coefficient of variation for the fraction of proteing binding 
PB.CV=0.1
...
## Standard deviation for organ volume
...
KbileC.sd = K.CV*KbileC.mean         ## Standard deviation for bile excretion rate
...
#PC of the rest of body to blood
...
idata_S_CTC<- data_frame(ID=1:N) %>% 
  mutate(
    ...
    # Fraction of blood flow to the liver
    QLCa = rlnormTrunc(
      N,
      meanlog=m.log.QLCa,
      sdlog=sd.log.QLCa,
...
##---------Scenario A : Dose 22.05 mg/kg/12h 5 days-------------
Sim_S_B_CTC <-pred (pars = Pars_CTC, idata = idata_S_CTC, 
                    tinterval = 12, Dose = 11.025, 
                    Dtimes = 10, route = 'oral')

CTC_pop_99<-Sim_S_B_CTC %>% group_by (Time) %>% summarise (
  M99 = quantile(CM, probs  = 0.99),
  L99 = quantile(CL, probs  = 0.99),
  K99 = quantile(CK, probs  = 0.99),
  F99= quantile(CF, probs  = 0.99))

CTC_pop_95<-Sim_S_B_CTC %>% group_by (Time) %>% summarise (
...
WDIs_CTC_L_USA <- CTC_pop_99 %>% filter(round(L99,3) <= TOL_USA[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_K_USA <- CTC_pop_99 %>% filter(round(K99,3) <= TOL_USA[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_M_USA <- CTC_pop_99 %>% filter(round(M99,3) <= TOL_USA[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_F_USA <- CTC_pop_99 %>% filter(round(F99,3) <= TOL_USA[4] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,Canada
TOL_Ca<-c(0.6,1.2,0.2)
WDIs_CTC_L_Ca <- CTC_pop_99 %>% filter(round(L99,3) <= TOL_Ca[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_K_Ca <- CTC_pop_99 %>% filter(round(K99,3) <= TOL_Ca[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_M_Ca <- CTC_pop_99 %>% filter(round(M99,3) <= TOL_Ca[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,F,Japan
TOL_Ja<-c(0.6,1,0.2,0.2)
WDIs_CTC_L_Ja <- CTC_pop_99 %>% filter(round(L99,3) <= TOL_Ja[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_K_Ja <- CTC_pop_99 %>% filter(round(K99,3) <= TOL_Ja[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_M_Ja <- CTC_pop_99 %>% filter(round(M99,3) <= TOL_Ja[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_F_Ja <- CTC_pop_99 %>% filter(round(F99,3) <= TOL_Ja[4] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,EU
TOL_EU<-c(0.3,0.6,0.1)
WDIs_CTC_L_EU <- CTC_pop_95 %>% filter(round(L95,3) <= TOL_EU[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_K_EU <- CTC_pop_95 %>% filter(round(K95,3) <= TOL_EU[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_M_EU <- CTC_pop_95 %>% filter(round(M95,3) <= TOL_EU[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,China
TOL_CH<-c(0.6,1.2,0.2)
WDIs_CTC_L_CH <- CTC_pop_95 %>% filter(round(L95,3) <= TOL_CH[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_K_CH <- CTC_pop_95 %>% filter(round(K95,3) <= TOL_CH[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_CTC_M_CH <- CTC_pop_95 %>% filter(round(M95,3) <= TOL_CH[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
...
###Figure for depletion for usa 
p_CTC_S_B_L_USA<- MCplot(Sim_S_B_CTC , target = "Liver", 
                         TOL =TOL_USA[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_CTC_S_B_K_USA<- MCplot(Sim_S_B_CTC , target = "Kidney",
                         TOL =TOL_USA[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_CTC_S_B_M_USA<- MCplot(Sim_S_B_CTC , target = "Muscle",
                         TOL =TOL_USA[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_CTC_S_B_F_USA<- MCplot(Sim_S_B_CTC , target = "Fat",
                         TOL =TOL_USA[4] , tdoses = 10,tinterval=12, Y.limit = 1e-5, color = 'red')

## Combine the figures
p1_CTC_USA <-p_CTC_S_B_L_USA+p_CTC_S_B_K_USA+p_CTC_S_B_M_USA +p_CTC_S_B_F_USA
...
###Figure for depletion for Ca 
p_CTC_S_B_L_Ca<- MCplot(Sim_S_B_CTC , target = "Liver", 
                        TOL =TOL_Ca[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_CTC_S_B_K_Ca<- MCplot(Sim_S_B_CTC , target = "Kidney",
                        TOL =TOL_Ca[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_CTC_S_B_M_Ca<- MCplot(Sim_S_B_CTC , target = "Muscle",
                        TOL =TOL_Ca[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
## Summarize 
p1_CTC_Ca <-p_CTC_S_B_L_Ca+p_CTC_S_B_K_Ca+p_CTC_S_B_M_Ca
...
p_CTC_S_B_L_Ja <- MCplot(Sim_S_B_CTC , target = "Liver", 
                            TOL =TOL_Ja [1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_CTC_S_B_K_Ja <- MCplot(Sim_S_B_CTC , target = "Kidney",
                            TOL =TOL_Ja [2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_CTC_S_B_M_Ja <- MCplot(Sim_S_B_CTC , target = "Muscle",
                            TOL =TOL_Ja [3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_CTC_S_B_F_Ja<- MCplot(Sim_S_B_CTC , target = "Fat",
                           TOL =TOL_Ja[4] , tdoses = 10,tinterval=12, Y.limit = 1e-5, color = 'red')
## Combine the figures 
p1_CTC_Ja <-p_CTC_S_B_L_Ja+p_CTC_S_B_K_Ja+p_CTC_S_B_M_Ja+p_CTC_S_B_F_Ja
...
###Figure for depletion for EU 
p_CTC_S_B_L_EU <- MCplot_95(Sim_S_B_CTC , target = "Liver", 
                            TOL =TOL_EU[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_CTC_S_B_K_EU <- MCplot_95(Sim_S_B_CTC , target = "Kidney",
                            TOL =TOL_EU[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_CTC_S_B_M_EU <- MCplot_95(Sim_S_B_CTC , target = "Muscle",
                            TOL =TOL_EU[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  

## Combine the figures 
p1_CTC_EU <-p_CTC_S_B_L_EU+p_CTC_S_B_K_EU+p_CTC_S_B_M_EU
...
###Figure for depletion for China 
p_CTC_S_B_L_CH <- MCplot_95(Sim_S_B_CTC , target = "Liver", 
                            TOL =TOL_CH[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_CTC_S_B_K_CH <- MCplot_95(Sim_S_B_CTC , target = "Kidney",
                            TOL =TOL_CH[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_CTC_S_B_M_CH <- MCplot_95(Sim_S_B_CTC , target = "Muscle",
                            TOL =TOL_CH[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  

p1_CTC_CH <-p_CTC_S_B_L_CH+p_CTC_S_B_K_CH+p_CTC_S_B_M_CH
...
P_2_SUMMARY<-p_CTC_S_B_L_Ca+p_CTC_S_B_K_Ca+p_CTC_S_B_M_Ca+
  p_CTC_S_B_L_CH+p_CTC_S_B_K_CH+p_CTC_S_B_M_CH+
  p_CTC_S_B_L_EU+p_CTC_S_B_K_EU+p_CTC_S_B_M_EU+
  p_CTC_S_B_L_Ja+p_CTC_S_B_K_Ja+p_CTC_S_B_M_Ja+p_CTC_S_B_F_Ja+
  p_CTC_S_B_L_USA+p_CTC_S_B_K_USA+p_CTC_S_B_M_USA +p_CTC_S_B_F_USA

# ggsave("Figure 22 mg_kg for 5 days via POW-CTC.tiff",scale = 1.5,
#        plot = P_2_SUMMARY,

...
Pars_OTC <- c(
  ...
  PF          = 0.2,
  PRest       = 3.0)
###---Parameter setting for Population model---####### 
...
#Fraction of protein binding drug in plasma
PB.mean        = 0.709
...
# Coefficient of variation for percentage of protein binding sd/mean 
PB.CV          = 0.1
...
idata_S_OTC<- data_frame(ID=1:N) %>% 
  mutate(
    ...
    QCC = rtruncnorm(
      ...
      mean = QCC.mean,sd = QCC.sd),  # Cardiac output index (L/h/kg)
    # Fraction of protein binding drug in plasma
    PB = rlnormTrunc(
      N,
      meanlog = m.log.PB,
    ...
    # Fracion of blood flow to liver (unitless) 
    QLCa = rlnormTrunc(
      N,
      meanlog=m.log.QLCa,
      sdlog=sd.log.QLCa,
...
## Define the function for predicting drug concentration in tissues
...
MCplot <- function(data, target, TOL, tdoses, tinterval = 12,
  ...
  endtime <-x.intercept +(tdoses * tinterval / 24 )
...
Sim_S_A_OTC <-pred (pars = Pars_OTC, idata = idata_S_OTC, 
                    tinterval = 12, Dose = 11.025, 
                    Dtimes = 14, route = 'oral')
# Determination of 99th percentile 
OTC_pop_99<-Sim_S_A_OTC %>% group_by (Time) %>% summarise (
  M99 = quantile(CM, probs  = 0.99),
  L99 = quantile(CL, probs  = 0.99),
  K99 = quantile(CK, probs  = 0.99),
  F99= quantile(CF, probs   = 0.99))
# Determination of 95th percentile 
OTC_pop_95<-Sim_S_A_OTC %>% group_by (Time) %>% summarise (
...
WDIs_OTC_L_USA <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_USA[1] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_USA <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_USA[2] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_USA <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_USA[3] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_F_USA <- OTC_pop_99 %>% filter(round(F99,3) <= TOL_USA[4] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()

#L,K,M,Canada
TOL_Ca<-c(0.6,1.2,0.2)
WDIs_OTC_L_Ca <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_Ca[1] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_Ca <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_Ca[2] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_Ca <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_Ca[3] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()

#L,K,M,F,Japan
TOL_Ja<-c(0.6,1,0.2,0.2)
WDIs_OTC_L_Ja <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_Ja[1] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_Ja <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_Ja[2] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_Ja <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_Ja[3] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_F_Ja <- OTC_pop_99 %>% filter(round(F99,3) <= TOL_Ja[4] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()

#L,K,M, EU
TOL_EU<-c(0.3,0.6,0.1)
WDIs_OTC_L_EU <- OTC_pop_95 %>% filter(round(L95,3) <= TOL_EU[1] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_EU <- OTC_pop_95 %>% filter(round(K95,3) <= TOL_EU[2] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_EU <- OTC_pop_95 %>% filter(round(M95,3) <= TOL_EU[3] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()

#L,K,M, China
TOL_CH<-c(0.6,1.2,0.2)
WDIs_OTC_L_CH <- OTC_pop_95 %>% filter(round(L95,3) <= TOL_CH[1] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_CH <- OTC_pop_95 %>% filter(round(K95,3) <= TOL_CH[2] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_CH <- OTC_pop_95 %>% filter(round(M95,3) <= TOL_CH[3] & Time >=24*6.5)%>%mutate(WDIs = (Time/24)-6.5)%>%select(WDIs)%>%min()

# Summary the withdrawal results
WDIS_TOTAL_USA<-c(WDIs_OTC_L_USA,WDIs_OTC_K_USA,WDIs_OTC_M_USA,WDIs_OTC_F_USA)
WDIS_TOTAL_Ca<-c(WDIs_OTC_L_Ca,WDIs_OTC_K_Ca,WDIs_OTC_M_Ca)
WDIS_TOTAL_Ja<-c(WDIs_OTC_L_Ja,WDIs_OTC_K_Ja,WDIs_OTC_M_Ja,WDIs_OTC_F_Ja)
WDIS_TOTAL_EU<-c(WDIs_OTC_L_EU,WDIs_OTC_K_EU,WDIs_OTC_M_EU)
WDIS_TOTAL_CH<-c(WDIs_OTC_L_CH,WDIs_OTC_K_CH,WDIs_OTC_M_CH)
...
###Figure for residue depletion for US
p_OTC_S_A_L_USA<- MCplot(Sim_S_A_OTC , target = "Liver", 
                         TOL =TOL_USA[1] , tdoses = 14, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_A_K_USA<- MCplot(Sim_S_A_OTC , target = "Kidney",
                         TOL =TOL_USA[2] , tdoses = 14, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_A_M_USA<- MCplot(Sim_S_A_OTC , target = "Muscle",
                         TOL =TOL_USA[3], tdoses =14, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_OTC_S_A_F_USA<- MCplot(Sim_S_A_OTC , target = "Fat",
                         TOL =TOL_USA[4] , tdoses = 14,tinterval=12, Y.limit = 1e-5, color = 'red')

## Combine the figures
p1_OTC_USA <-p_OTC_S_A_L_USA+p_OTC_S_A_K_USA+p_OTC_S_A_M_USA +p_OTC_S_A_F_USA
...
###Figure for residue depletion for Canada
p_OTC_S_A_L_Ca<- MCplot(Sim_S_A_OTC , target = "Liver", 
                        TOL =TOL_Ca[1] , tdoses = 14, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_A_K_Ca<- MCplot(Sim_S_A_OTC , target = "Kidney",
                        TOL =TOL_Ca[2] , tdoses = 14, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_A_M_Ca<- MCplot(Sim_S_A_OTC , target = "Muscle",
                        TOL =TOL_Ca[3], tdoses =14, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
## Combine the figures
p1_OTC_Ca <-p_OTC_S_A_L_Ca+p_OTC_S_A_K_Ca+p_OTC_S_A_M_Ca

# # ## Save figures
# ggsave("Fig WDT_Canada_A.tiff",scale = 1.5,
#        plot = p1_OTC_Ca,
#        width = 30, height = 8, units = "cm", dpi=320)

###Figure for residue depletion for Janpan
p_OTC_S_A_L_Ja <- MCplot(Sim_S_A_OTC , target = "Liver", 
                         TOL =TOL_Ja [1] , tdoses = 14, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_A_K_Ja <- MCplot(Sim_S_A_OTC , target = "Kidney",
                         TOL =TOL_Ja [2] , tdoses = 14, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_A_M_Ja <- MCplot(Sim_S_A_OTC , target = "Muscle",
                         TOL =TOL_Ja [3], tdoses =14, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_OTC_S_A_F_Ja<- MCplot(Sim_S_A_OTC , target = "Fat",
                        TOL =TOL_Ja[4] , tdoses = 14,tinterval=12, Y.limit = 1e-5, color = 'red')
## Combine the figures
p1_OTC_Ja <-p_OTC_S_A_L_Ja+p_OTC_S_A_K_Ja+p_OTC_S_A_M_Ja+p_OTC_S_A_F_Ja
...
p_OTC_S_A_L_EU <- MCplot_95(Sim_S_A_OTC , target = "Liver", 
                            TOL =TOL_EU[1] , tdoses = 14, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_A_K_EU <- MCplot_95(Sim_S_A_OTC , target = "Kidney",
                            TOL =TOL_EU[2] , tdoses = 14, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_A_M_EU <- MCplot_95(Sim_S_A_OTC , target = "Muscle",
                            TOL =TOL_EU[3], tdoses =14, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
##Combine the figures
p1_OTC_EU <-p_OTC_S_A_L_EU+p_OTC_S_A_K_EU+p_OTC_S_A_M_EU
...
p_OTC_S_A_L_CH <- MCplot_95(Sim_S_A_OTC , target = "Liver", 
                            TOL =TOL_CH[1] , tdoses = 14, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_A_K_CH <- MCplot_95(Sim_S_A_OTC , target = "Kidney",
                            TOL =TOL_CH[2] , tdoses = 14, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_A_M_CH <- MCplot_95(Sim_S_A_OTC , target = "Muscle",
                            TOL =TOL_CH[3], tdoses =14, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
#Combine the figures
p1_OTC_CH <-p_OTC_S_A_L_CH+p_OTC_S_A_K_CH+p_OTC_S_A_M_CH
...
P_1_SUMMARY<-p_OTC_S_A_L_Ca+p_OTC_S_A_K_Ca+p_OTC_S_A_M_Ca+
  p_OTC_S_A_L_CH+p_OTC_S_A_K_CH+p_OTC_S_A_M_CH+
  p_OTC_S_A_L_EU+p_OTC_S_A_K_EU+p_OTC_S_A_M_EU+
  p_OTC_S_A_L_Ja+p_OTC_S_A_K_Ja+p_OTC_S_A_M_Ja+p_OTC_S_A_F_Ja+
  p_OTC_S_A_L_USA+p_OTC_S_A_K_USA+p_OTC_S_A_M_USA +p_OTC_S_A_F_USA
  

ggsave("Figure 22 mg_kg for 7 days via PO-OTC.tiff",scale = 1.5,
       plot = P_1_SUMMARY,
       width = 40,height = 20,units = "cm", dpi=320)

#--------------------Scenario B : Dose 22.05 mg/kg/12h 5 days via oral with water--------------#############
Sim_S_B_OTC <-pred (pars = Pars_OTC, idata = idata_S_OTC, 
                    tinterval = 12, Dose = 11.025, 
                    Dtimes = 10, route = 'oral')
#Determination of 99th percentile
OTC_pop_99<-Sim_S_B_OTC %>% group_by (Time) %>% summarise (
  M99 = quantile(CM, probs  = 0.99),
  L99 = quantile(CL, probs  = 0.99),
  K99 = quantile(CK, probs  = 0.99),
  F99= quantile(CF, probs  = 0.99))
#Determination of 95th percentile
OTC_pop_95<-Sim_S_B_OTC %>% group_by (Time) %>% summarise (
  M95 = quantile(CM, probs  = 0.95),
  L95 = quantile(CL, probs  = 0.95),
  K95 = quantile(CK, probs  = 0.95),
  F95= quantile(CF, probs  = 0.95))
...
WDIs_OTC_L_USA <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_USA[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_USA <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_USA[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_USA <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_USA[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_F_USA <- OTC_pop_99 %>% filter(round(F99,3) <= TOL_USA[4] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,Canada
TOL_Ca<-c(0.6,1.2,0.2)

WDIs_OTC_L_Ca <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_Ca[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_Ca <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_Ca[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_Ca <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_Ca[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,F,Japanese
TOL_Ja<-c(0.6,1,0.2,0.2)
WDIs_OTC_L_Ja <- OTC_pop_99 %>% filter(round(L99,3) <= TOL_Ja[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_Ja <- OTC_pop_99 %>% filter(round(K99,3) <= TOL_Ja[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_Ja <- OTC_pop_99 %>% filter(round(M99,3) <= TOL_Ja[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_F_Ja <- OTC_pop_99 %>% filter(round(F99,3) <= TOL_Ja[4] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,EU
TOL_EU<-c(0.3,0.6,0.1)

WDIs_OTC_L_EU <- OTC_pop_95 %>% filter(round(L95,3) <= TOL_EU[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_EU <- OTC_pop_95 %>% filter(round(K95,3) <= TOL_EU[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_EU <- OTC_pop_95 %>% filter(round(M95,3) <= TOL_EU[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()

#L,K,M,China
TOL_CH<-c(0.6,1.2,0.2)

WDIs_OTC_L_CH <- OTC_pop_95 %>% filter(round(L95,3) <= TOL_CH[1] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_K_CH <- OTC_pop_95 %>% filter(round(K95,3) <= TOL_CH[2] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
WDIs_OTC_M_CH <- OTC_pop_95 %>% filter(round(M95,3) <= TOL_CH[3] & Time >=24*4.5)%>%mutate(WDIs = (Time/24)-4.5)%>%select(WDIs)%>%min()
...
p_OTC_S_B_L_USA<- MCplot(Sim_S_B_OTC , target = "Liver", 
                         TOL =TOL_USA[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_B_K_USA<- MCplot(Sim_S_B_OTC , target = "Kidney",
                         TOL =TOL_USA[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_B_M_USA<- MCplot(Sim_S_B_OTC , target = "Muscle",
                         TOL =TOL_USA[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_OTC_S_B_F_USA<- MCplot(Sim_S_B_OTC , target = "Fat",
                         TOL =TOL_USA[4] , tdoses = 10,tinterval=12, Y.limit = 1e-5, color = 'red')

## Combine the figures 
p1_OTC_USA <-p_OTC_S_B_L_USA+p_OTC_S_B_K_USA+p_OTC_S_B_M_USA +p_OTC_S_B_F_USA

# # ## Save figures
# ggsave("Fig WDT_USA_B.tiff",scale = 1.5,
#        plot = p1_OTC_USA,
#        width = 25, height = 15, units = "cm", dpi=320)

###Figure for residue depletion for Ca 
p_OTC_S_B_L_Ca<- MCplot(Sim_S_B_OTC , target = "Liver", 
                        TOL =TOL_Ca[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_B_K_Ca<- MCplot(Sim_S_B_OTC , target = "Kidney",
                        TOL =TOL_Ca[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_B_M_Ca<- MCplot(Sim_S_B_OTC , target = "Muscle",
                        TOL =TOL_Ca[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
## Combine the figures
p1_OTC_Ca <-p_OTC_S_B_L_Ca+p_OTC_S_B_K_Ca+p_OTC_S_B_M_Ca

# # ## Save figures
# ggsave("Fig WDT_Canada_B.tiff",scale = 1.5,
#        plot = p1_OTC_Ca,
#        width = 30, height = 8, units = "cm", dpi=320)

###Figure for residue depletion for Ja 
p_OTC_S_B_L_Ja <- MCplot(Sim_S_B_OTC , target = "Liver", 
                         TOL =TOL_Ja [1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_B_K_Ja <- MCplot(Sim_S_B_OTC , target = "Kidney",
                         TOL =TOL_Ja [2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_B_M_Ja <- MCplot(Sim_S_B_OTC , target = "Muscle",
                         TOL =TOL_Ja [3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
p_OTC_S_B_F_Ja<- MCplot(Sim_S_B_OTC , target = "Fat",
                        TOL =TOL_Ja[4] , tdoses = 10,tinterval=12, Y.limit = 1e-5, color = 'red')
## Combine the figures
p1_OTC_Ja <-p_OTC_S_B_L_Ja+p_OTC_S_B_K_Ja+p_OTC_S_B_M_Ja+p_OTC_S_B_F_Ja

# # ## Save figures
# ggsave("Fig WDT_Japanese_B.tiff",scale = 1.5,
#        plot = p1_OTC_Ja,
#        width = 25, height = 15, units = "cm", dpi=320)


###Figure for residue depletion for EU 
p_OTC_S_B_L_EU <- MCplot_95(Sim_S_B_OTC , target = "Liver", 
                            TOL =TOL_EU[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_B_K_EU <- MCplot_95(Sim_S_B_OTC , target = "Kidney",
                            TOL =TOL_EU[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_B_M_EU <- MCplot_95(Sim_S_B_OTC , target = "Muscle",
                            TOL =TOL_EU[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  

## Combine the figures
p1_OTC_EU <-p_OTC_S_B_L_EU+p_OTC_S_B_K_EU+p_OTC_S_B_M_EU

# # ## Save figures
# ggsave("Fig WDT_EU_B.tiff",scale = 1.5,
#        plot = p1_OTC_EU,
#        width = 30, height = 8, units = "cm", dpi=320)

###Figure for residue depletion for China 
p_OTC_S_B_L_CH <- MCplot_95(Sim_S_B_OTC , target = "Liver", 
                            TOL =TOL_CH[1] , tdoses = 10, tinterval=12, Y.limit = 1e-5) 
p_OTC_S_B_K_CH <- MCplot_95(Sim_S_B_OTC , target = "Kidney",
                            TOL =TOL_CH[2] , tdoses = 10, tinterval=12,Y.limit = 1e-5, color = 'bisque3')  
p_OTC_S_B_M_CH <- MCplot_95(Sim_S_B_OTC , target = "Muscle",
                            TOL =TOL_CH[3], tdoses =10, tinterval=12,Y.limit = 1e-5, color = 'yellow')  
#Combine the figures
p1_OTC_CH <-p_OTC_S_B_L_CH+p_OTC_S_B_K_CH+p_OTC_S_B_M_CH


# # ## Save figures
# ggsave("Fig WDT_CH_B.tiff",scale = 1.5,
#        plot = p1_OTC_CH,
#        width = 30, height = 8, units = "cm", dpi=320)
# 

P_2_SUMMARY<-p_OTC_S_B_L_Ca+p_OTC_S_B_K_Ca+p_OTC_S_B_M_Ca+
  p_OTC_S_B_L_CH+p_OTC_S_B_K_CH+p_OTC_S_B_M_CH+
  p_OTC_S_B_L_EU+p_OTC_S_B_K_EU+p_OTC_S_B_M_EU+
  p_OTC_S_B_L_Ja+p_OTC_S_B_K_Ja+p_OTC_S_B_M_Ja+p_OTC_S_B_F_Ja+
  p_OTC_S_B_L_USA+p_OTC_S_B_K_USA+p_OTC_S_B_M_USA +p_OTC_S_B_F_USA

# ggsave("Figure 22 mg_kg for 5 days via POW-OTC.tiff",scale = 1.5,
#        plot = P_2_SUMMARY,
#        width = 40,height = 20,units = "cm", dpi=320)
